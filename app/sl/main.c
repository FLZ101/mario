...
void restore_tty() {
  if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &g.orig_termios) == -1)
    die("tcsetattr");
  write(STDOUT_FILENO, "\x1b[?1049l", 8); // use normal screen buffer
...
void init() {
  if (tcgetattr(STDIN_FILENO, &g.orig_termios) == -1)
    die("tcgetattr");
  write(STDOUT_FILENO, "\x1b[?1049h", 8); // use alternate screen buffer
  ...
  if (tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) == -1)
    die("tcsetattr");
  if (get_window_size(&g.rows, &g.cols) == -1)
    die("get_window_size");
}
